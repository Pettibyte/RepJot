# RepJot browser capability report

## Run information

| Field | Value |
| --- | --- |
| Started | 2026-08-22T17:33:28.177Z |
| Finished | 2026-08-22T17:33:28.390Z |
| URL | https://repjot.com/capabilities.html |

## Summary

| Result | Count |
| --- | ---: |
| Supported | 67 |
| Unsupported | 9 |
| Limited or Error | 0 |
| Informational | 14 |
| Total checks displayed | 90 |

## Browser and display

| Capability | Result | Details |
| --- | --- | --- |
| User agent | Info | Mozilla/5.0 (Linux; Android 9; Kindle Build/1.0) AppleWebKit/537.36 (KHTML, like Gecko) Silk/80.4.14 like Chrome/80.0.3987.119 Safari/537.36 |
| Platform | Info | Linux armv7l |
| Language | Info | en-US |
| Viewport | Info | 930 × 1132 CSS pixels |
| Screen | Info | 930 × 1240 pixels; color depth 24 |
| Pixel ratio | Info | 2 |
| Online status | Info | true |
| Cookies enabled | Info | true |
| Logical processors | Info | 2 |
| Device memory | Info | 0.5 GiB (approximate) |

## JavaScript

| Capability | Result | Details |
| --- | --- | --- |
| Promises | Supported | Available |
| Fetch | Supported | Available |
| Map and Set | Supported | Available |
| WeakMap and WeakSet | Supported | Available |
| Symbol | Supported | Available |
| Proxy and Reflect | Supported | Available |
| globalThis | Supported | Available |
| MutationObserver | Supported | Available |
| queueMicrotask | Supported | Available |
| String.replaceAll | Unsupported | Not available |
| Array.from and includes | Supported | Available |
| BigInt | Supported | Available |
| WebAssembly | Unsupported | Not available |
| Internationalization (Intl) | Supported | Available |
| ES modules | Supported | Module scripts recognized |
| Arrow functions | Supported | Syntax parsed successfully |
| Classes | Supported | Syntax parsed successfully |
| Async functions | Supported | Syntax parsed successfully |
| Object and array spread | Supported | Syntax parsed successfully |
| Private class fields | Supported | Syntax parsed successfully |
| Optional chaining | Unsupported | Unexpected token . |
| Nullish coalescing | Unsupported | Unexpected token ? |

## HTML and CSS

| Capability | Result | Details |
| --- | --- | --- |
| querySelector | Supported | Available |
| Template element | Supported | Available |
| Details element | Supported | Available |
| Dialog element | Supported | Available |
| Constraint validation | Supported | Available |
| Input type: number | Supported | Browser recognizes this input type |
| Input type: date | Supported | Browser recognizes this input type |
| Input type: time | Supported | Browser recognizes this input type |
| Input type: range | Supported | Browser recognizes this input type |
| CSS feature queries | Supported | Available |
| Flexbox | Supported | Available |
| CSS Grid | Supported | Available |
| Custom properties | Supported | Available |
| Sticky positioning | Supported | Available |
| Media queries | Supported | Available |

## Input

| Capability | Result | Details |
| --- | --- | --- |
| Touch points | Unsupported | maxTouchPoints: 0; touch events exposed: false |
| Pointer events | Supported | Available |
| Keyboard events | Supported | Available |
| Selection API | Supported | Available |
| Primary pointer | Info | fine |
| Hover input | Info | available |

## Storage

| Capability | Result | Details |
| --- | --- | --- |
| localStorage | Supported | Write, read, and remove succeeded |
| sessionStorage | Supported | Write, read, and remove succeeded |
| IndexedDB | Supported | API is present; an open/write check will follow |
| Cache Storage | Supported | Available |
| IndexedDB open test | Supported | Temporary database opened successfully and deletion was requested |

## Networking and security

| Capability | Result | Details |
| --- | --- | --- |
| XMLHttpRequest | Supported | Available |
| WebSocket | Supported | Available |
| Server-sent events | Supported | Available |
| Beacon | Supported | Available |
| Secure context | Supported | Page is a secure context |
| URL API | Supported | Available |
| URLSearchParams | Supported | Available |
| Web Crypto | Supported | crypto.subtle is present |
| Random UUID | Unsupported | crypto.randomUUID is absent |
| Popup API | Supported | window.open is exposed; popup policy still requires a user gesture |

## Files and sharing

| Capability | Result | Details |
| --- | --- | --- |
| Blob | Supported | Available |
| File | Supported | Available |
| FileReader | Supported | Available |
| Object URLs | Supported | Available |
| Download attribute | Supported | Available |
| Web Share | Unsupported | Not available |
| Async Clipboard | Supported | Available |

## Graphics and media

| Capability | Result | Details |
| --- | --- | --- |
| Canvas 2D | Supported | Available |
| WebGL | Unsupported | Not available |
| SVG | Supported | Available |
| Video codecs | Info | H.264: no; WebM VP8: probably |
| Audio codecs | Info | MP3: probably; AAC: no |
| Web Audio | Supported | Available |

## Background and device APIs

| Capability | Result | Details |
| --- | --- | --- |
| Web Workers | Supported | Available |
| Service Workers | Supported | API is exposed; registration was not attempted |
| Page visibility | Supported | Available |
| Notifications | Supported | API is exposed; permission was not requested |
| Geolocation | Supported | API is exposed; permission was not requested |
| Vibration | Supported | Available |
| Fullscreen | Supported | Available |
| Screen orientation | Supported | Available |
| Wake Lock | Unsupported | Not available |

Generated locally by RepJot. No results were transmitted.
