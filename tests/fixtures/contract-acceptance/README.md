...
| `workouts.deep-nested.json` | Positive nesting regression fixture (P10-D007): one depth-16 chain of containers cycling through all five strategies (`sequence`, `rounds`, `amrap`, `emom`, `complex`) over two leaf exercises. Must validate against the workouts schema without exponential branch exploration or any product depth limit. |
