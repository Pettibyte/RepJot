...
`src/ui/screens/`. Follow it for the editable tree and every control in it.
...
8. Expose a two-way bound prop with `$bindable()`. `ValueInput.svelte` binds its
   field text that way. Do not hand-roll a `value` plus `oninput` pair to copy
   what `$bindable()` already does.
